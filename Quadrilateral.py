    ...
    def __init__(self,A:Point2D,B:Point2D,C:Point2D,D:Point2D):
        if(type(A) is not Point2D or
           ...
           type(D) is not Point2D):
            raise TypeError('BNad type for initial ata , all ata must be Point2D objects')
        ...
        if(self.AB<=0.0 or
           ...
           self.CD<=0.0 or
           self.DA<=0.0):
            raise ValueError('Input points do not form a valid quarilateral: zero or negative side length ')    
        ...
        if(
            ...
            raise ValueError('Input points do not form a valid quarilateral : sides violet traingular inequality')
...
def draw_point(p):
    r=4
    #raw red point
    canvas_handle.create_oval(p.x-r,p.y-r,p.x+r,p.y+r,fill='red',outline='red')
...
def onArea():
    ...
    try:
        if len(points)!=4:
            raise ValueError('Quarilateral is not complete')
        Q=Quadrilateral(points[0],points[1],points[2],points[3])
...
def onClear():
    global points,tenp_line,output_var
...
def Quadrilater_App():
    ...
    root_window=Tk()
    root_window.title('Quarilateral Drawer')
    ...
    output_var.set('Output will appear here')
